...
def test_validate_args_requires_scoped_promoted_single_device_pcie():
    ...
    bad = (
        ({"allow_pcie_load": False}, "--allow-pcie-load"),
        ({"device_id": 1}, "only --device-id 0"),
        ({"chips": ("bm1690",)}, "only for explicit --chip sg2260e"),
        ({"operations": None}, "explicit --op/--case subset"),
        ({"bm_cmodel_summary": None}, "--bm-cmodel-summary"),
        ({"all_pcie_cases": True}, "cannot be combined"),
    )
...
def test_validate_args_accepts_only_explicit_unfiltered_full_pcie_matrix():
    matrix._validate_args(
        _args("pcie", operations=None, all_pcie_cases=True))
...
def test_validate_numeric_identity_requires_exact_case_and_backend():
    ...
    matrix._validate_numeric_identity(
        payload, case=case, chip="sg2260e", runtime_mode="pcie")
...
def test_promotion_requires_matching_exact_bm_and_sg_results(monkeypatch, tmp_path):
    ...
    broken["results"][0]["case"] = matrix._json_normalized(
        _case("sub.float16.dense").to_json())
...
def test_pcie_matrix_pins_snapshot_guards_each_case_and_checks_board(
    ...
    monkeypatch.setattr(
        matrix, "assert_source_identity_unchanged",
        lambda _root, _summary: events.append("source-guard"))
    monkeypatch.setattr(
        matrix, "assert_toolchain_identity_unchanged",
    ...
    status = matrix._run_matrix(
        args, tmp_path, output_dir, ("sg2260e",), (case,),
        {"PPL_PROJECT_ROOT": "/sdk", "TMPDIR": str(scratch)})

    assert status == 0
    assert events == [
        "snapshot", "pin", "board", "source-guard", "toolchain-guard",
        "dispatch", "board",
    ]
...
def test_failed_postflight_is_not_retried(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(matrix, "toolchain_identity", lambda _env, _mode: dict(toolchain))
    monkeypatch.setattr(
        matrix, "pin_native_worker_libraries", lambda environment, _identity: dict(environment))
    monkeypatch.setattr(
        matrix, "_validate_pcie_promotion",
...
def test_pcie_worker_uses_fail_closed_supervisor_and_unsets_inherited_profile_controls(
    ...
    result = matrix._run_one(
        _args("pcie"),
        {"PPL_PROJECT_ROOT": "/sdk", "PYTHONPATH": "/snapshot"},
        tmp_path,
    ...
    assert observed["command"][-9:] == [
        matrix.sys.executable, "-u",
        "/snapshot/testing/python/jit/tpukernel_ops_worker.py",
        "--case-id", case.case_id, "--chip", "sg2260e", "--runtime-mode", "pcie",
    ]
