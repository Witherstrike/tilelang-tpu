...
''', {"T": T})
    ...
    wrapper.template_dir = str(root)
    wrapper.output_dir = str(root)
...
int tpuRtStreamSynchronize(void*) { return event("sync"); }
...
extern "C" int tilelang_tpu_bind_device(int);
int main() { tilelang_tpu_bind_device(3); float a[4]={}; void* args[]={a,a}; return tilelang_tpu_run(args); }
''')
    exe = root / "runner"
    subprocess.run(["g++", "-std=c++17", '-DTILELANG_PPL_KERNEL_PATH="unused"', f"-I{root}", str(root / "main.cpp"),
...
@pytest.mark.parametrize("failure,forbidden", [
    ...
    ("launch", "d2h"), ("sync", "d2h"), ("d2h", None), ("free", None), ("unload", None),
    ("destroy", None),
])
def test_runtime_failure_stops_work(host_runner, failure, forbidden):
    env = dict(os.environ, TILELANG_TPU_ALLOW_PCIE_LOAD="1", TILELANG_TPU_DEVICE_ID="3", FAIL_AT=failure)
...
def test_success_launches_once(host_runner):
    env = dict(os.environ, TILELANG_TPU_ALLOW_PCIE_LOAD="1", TILELANG_TPU_DEVICE_ID="3", FAIL_AT="")
    result = subprocess.run([host_runner], env=env, capture_output=True, text=True)
    assert result.returncode == 0, result.stderr
    assert [line for line in result.stdout.splitlines() if not line.startswith("Single kernel")] == ["init", "device=3", "set_device", "stream", "load",
                                         "malloc", "malloc", "h2d", "h2d", "launch", "kernel_launch success", "sync", "d2h",
...
def test_invalid_device_rejected_before_init(host_runner, device):
    env = dict(os.environ, TILELANG_TPU_ALLOW_PCIE_LOAD="1", TILELANG_TPU_DEVICE_ID=device, FAIL_AT="")
    result = subprocess.run([host_runner], env=env, capture_output=True, text=True)
    assert result.returncode != 0
