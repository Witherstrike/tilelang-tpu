...
def main(A: T.Buffer((16,), "float16"), B: T.Buffer((16,), "float16")):
    ...
    T.evaluate(T.call_extern("int32", "ppl.gemm", T.tvm_access_ptr(T.type_annotation("float16"), C.data, 0, 16, 1), T.tvm_access_ptr(T.type_annotation("float16"), D.data, 0, 16, 1), T.tvm_access_ptr(T.type_annotation("float16"), C.data, 0, 16, 2)))
...
def test_rv_legalization_covers_supported_operator_families():
