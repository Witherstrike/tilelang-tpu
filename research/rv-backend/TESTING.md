...
PCIe 矩阵是 fail-stop：一个 case 超时、加载失败、数值不符、没有 trace 或解析失败，立即记录部分结果并停止后续测试。它需要显式 `--allow-pcie --allow-pcie-profile --device-id <n>`；不可因设备风险跳过这些门禁。
...
静态/单元回归为 `101 passed, 4 skipped`，覆盖 target capability、PPL layout、模型隔离、raw ABI 混用拒绝、copy/cast/layout 的失败闭合、生成源码隔离、PCIe compile/link（不加载硬件）、profiling 超时清理以及 AddressAssign 的 GEMM read/write effect。4 个 skip 均需要独立外部工具或显式硬件授权。

## 3. PCIe 最终矩阵

所有 case 都在 device 0 上 fresh-compile、单次 launch、回传并完成数值比较；decoder 状态均为 `ready`。逐条 `begin/end/duration/opcode` 位于 ignored 的
...
4. 检查 `summary.json` 的 `complete=true`、每 case `status=passed`、数值 worker 成功、`raw_trace_file_count>0`、`parser_status=ready`、`timed_instruction_count>0`，并验证所有 interval 非负。
