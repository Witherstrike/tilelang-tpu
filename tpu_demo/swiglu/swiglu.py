...
def build_swiglu(*, rows: int = 8, width: int = 32, block_rows: int = 4,
    ...
    validate_exact_tiling(
        "swiglu", ("rows", rows, block_rows), ("width", width, block_width))
    @T.prim_func
    def kernel(gate: T.Tensor((rows, width), dtype),
               up: T.Tensor((rows, width), dtype),
        ...
        with T.Kernel(T.ceildiv(rows, block_rows), T.ceildiv(width, block_width),
            ...
            if dtype == "float32":
                T.ppl_copy(output_f32,
                           destination[bx * block_rows, by * block_width])
            else:
...
def run(*, dtype: str, chip: str, programming_model: str, runtime_mode: str,
    ...
    timing = compile_and_launch(
        build_swiglu(rows=shape[0], width=shape[1], dtype=dtype),
        (gate, up, destination), chip=chip,
    ...
    return result_payload(
        operation="swiglu", dtype=dtype, chip=chip, programming_model=programming_model,
        runtime_mode=runtime_mode, metrics=metrics, timing=timing,
        parameters={"shape": list(shape), "block_rows": 4, "block_width": 32,
                    "accumulation_dtype": "float32", "seed": seed})
