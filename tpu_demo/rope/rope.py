...
def build_rope(*, rows: int = 8, width: int = 32, block_rows: int = 4,
               block_width: int = 32, dtype: str = "float16"):
    torch_dtype(dtype)
    validate_dimensions(
        "rope", rows=rows, width=width, block_rows=block_rows, block_width=block_width)
    validate_exact_tiling(
        "rope", ("rows", rows, block_rows), ("width", width, block_width))
    if width % 2 or block_width % 2:
        raise ValueError("RoPE width and block width must both be even")

    @T.prim_func
    def kernel(source: T.Tensor((rows, width), dtype),
               cosine: T.Tensor((rows, width), dtype),
               sine: T.Tensor((rows, width), dtype),
               destination: T.Tensor((rows, width), dtype)):
        with T.Kernel(T.ceildiv(rows, block_rows), T.ceildiv(width, block_width),
...
def _cosine_sine(rows: int, width: int, dtype: torch.dtype) -> Tuple[torch.Tensor,
                                                                      torch.Tensor]:
...
def _reference(source: torch.Tensor, cosine: torch.Tensor,
    ...
    output[:, 0::2] = (source_f32[:, 0::2] * cosine_f32[:, 0::2] -
                       source_f32[:, 1::2] * sine_f32[:, 1::2])
    output[:, 1::2] = (source_f32[:, 1::2] * cosine_f32[:, 1::2] +
                       source_f32[:, 0::2] * sine_f32[:, 0::2])
...
def run(*, dtype: str, chip: str, programming_model: str, runtime_mode: str,
    ...
    timing = compile_and_launch(
        build_rope(rows=rows, width=width, dtype=dtype),
        (source, cosine, sine, destination), chip=chip,
    ...
    return result_payload(
        operation="rope", dtype=dtype, chip=chip, programming_model=programming_model,
        runtime_mode=runtime_mode, metrics=metrics, timing=timing,
        parameters={"rows": rows, "width": width, "block_rows": 4,
                    "block_width": 32, "seed": seed})
